...
class ListBySearchView(APIView):
    ...
    def post(self, request, format=None):
        ...
        if price_range == '100 - 200':
            ...
            product_results = product_results.filter(price__gte=201)
            product_results = product_results.filter(price__lt=400)
        elif price_range == '401 - 600':
            product_results = product_results.filter(price__gte=401)
            product_results = product_results.filter(price__lt=600)
        elif price_range == '601 - 799':
            product_results = product_results.filter(price__gte=601)
